...
import { pickFilePath, type TimelineItemInput, type VideoTimelineOptions } from "../lib/tauri";
 ...
 */
export function VideoTimelineDialog({ open, photoIds, onClose }: VideoTimelineDialogProps) {
  ...
  const photoById = useMemo(() => new Map(activePhotos.map((p) => [p.id, p])), [activePhotos]);
  const isVideo = (id: string) => photoById.get(id)?.media_kind === "video";

  const [items, setItems] = useState<DraftItem[]>([]);
  const [transition, setTransition] = useState<"cut" | "cross_fade">("cross_fade");
  ...
  async function handleRender() {
    ...
    const options: VideoTimelineOptions = {
      ...
      height,
      fps,
      transitions: new Array(Math.max(0, timelineItems.length - 1)).fill(transition),
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-start justify-center bg-black/50 pt-16" onClick={onClose}>
      ...
      >
        <h2 className="mb-1 text-sm font-semibold text-text-primary">Video-Zeitachse</h2>
        <p className="mb-3 text-xs text-text-muted">
          {items.length} Eintrag{items.length === 1 ? "" : "e"} — Reihenfolge, Zuschnitt und Übergänge festlegen und als
          neues Video rendern.
        </p>
        ...
        <div className="mb-3 flex flex-col gap-2">
          {items.map((item, index) => {
            ...
            return (
              <div key={`${item.photoId}-${index}`} className="rounded border border-border p-2">
                <div className="mb-1 flex items-center justify-between gap-2">
                  <span className="min-w-0 flex-1 truncate text-xs text-text-primary" title={photo?.filename}>
                    {index + 1}. {photo?.filename ?? item.photoId} {video ? "(Video)" : "(Foto)"}
                  </span>
                ...
                {video ? (
                  <div className="flex gap-2">
                    <label className="flex flex-1 flex-col gap-1 text-xs text-text-secondary">
                      ...
                      <input
                        ...
                        onChange={(e) => updateItem(index, { inSeconds: Number(e.target.value) })}
                  ...
                  <label className="flex flex-col gap-1 text-xs text-text-secondary">
                    ...
                    <input
                      ...
                      onChange={(e) => updateItem(index, { holdSeconds: Number(e.target.value) })}
          ...
          {items.length === 0 && <p className="text-xs text-text-muted">Keine Einträge — Dialog schließen und Fotos/Videos auswählen.</p>}
        </div>

        <label className="mb-3 flex flex-col gap-1 text-xs text-text-secondary">
          Übergang (zwischen allen Einträgen)
          <select value={transition} onChange={(e) => setTransition(e.target.value as "cut" | "cross_fade")} className="rounded border border-border bg-bg-panel px-2 py-1 text-sm">
        ...
        {ffmpegAvailable === false && <p className="mb-2 text-xs text-text-muted">ffmpeg wurde nicht gefunden — Video-Export ist deaktiviert.</p>}
        {videoTimelineError && <p className="mb-2 text-xs text-danger">Fehler: {videoTimelineError}</p>}
        {!videoTimelineRunning && videoTimelineOutcome && <p className="mb-2 text-xs text-text-secondary">Gespeichert als „{videoTimelineOutcome.filename}“.</p>}
        ...
        <div className="flex justify-end gap-2">
          ...
          <button
            ...
            disabled={items.length === 0 || videoTimelineRunning || ffmpegAvailable !== true}
