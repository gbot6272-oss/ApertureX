...
fn main() {
    // Lokaler Fix gegenüber der veröffentlichten Version (siehe VENDORED.md
    // in diesem Verzeichnis): probiert zuerst pkg-config gegen eine bereits
    // installierte System-`libdlib` — `build()` unten enthält diesen
    // pkg-config-Pfad bereits, war aber in jeder veröffentlichten Version
    // (bis mindestens 20.0.1) toter Code, weil `main()` unbedingt zuerst
    // `download_and_unzip` aufrief. Dieser Block reiht die beiden bereits
    // vorhandenen Schritte nur um, ändert keine Logik.
