...
/// Dieselbe Luminanz-Gewichtung wie `builtin_luts::luminance` — eine
/// Konstante im ganzen Projekt statt mehrerer leicht abweichender.
...
/// Weiche Stufenfunktion (`smoothstep`) — überall dort verwendet, wo ein
/// harter Schwellwert sichtbare Kanten erzeugen würde.
...
/// Alphakanal bleibt unangetastet und wird am Ende wieder eingesetzt.
fn to_rgb_f32(rgba: &[u8]) -> Vec<f32> {
    let n = rgba.len() / 4;
...
fn write_back(rgb: &[f32], rgba: &mut [u8]) {
    let n = rgba.len() / 4;
    for i in 0..n {
        for c in 0..3 {
...
fn sample_map(map: &[u8], map_w: u32, map_h: u32, u: f32, v: f32) -> f32 {
