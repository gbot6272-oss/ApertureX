...
**Bedingte Presets (feste Feldliste, echter Regelbaum seit Phase 13 Schritt 7):** `PresetLeafCondition { field, op, value }` — feste Feldliste (ISO/Blende/Brennweite/Kameramodell/Objektiv, alle bereits in `photos`), Operatoren `>`/`<`/`=`/„enthält" (ADR-0031 Punkt 4, unverändert). Neu seit Phase 13 Schritt 7 (`DECISIONS.md` ADR-0040-Nachtrag V): `PresetRuleGroup { section, node }` — `node` ist ein beliebig tief verschachtelbarer UND/ODER-Baum (`lib/ruleTree.ts`s `RuleNode<TLeaf>`) statt einer einzelnen Bedingung, bearbeitet über den generischen `RuleTreeEditor.tsx`. `section: null` bedeutet weiterhin „gilt fürs ganze Preset" (ein Fehlschlag des Baums verhindert das Anwenden komplett); eine gesetzte Sektion grenzt einen Fehlschlag auf genau diese Sektion ein; mehrere `PresetRuleGroup`-Einträge bleiben untereinander UND-verknüpft. `evaluateRuleNode`/`applyRulesToSubset` laufen sowohl in `applyPreset`/`applyPresetStack` als auch in der Hover-Vorschau und dem `PresetThumbnail` — dieselbe Auswertung überall, damit die Vorschau nie etwas zeigt, was `applyPreset` tatsächlich nicht anwenden würde. Ein fehlendes Metadatum am aktuellen Foto gilt konservativ als nicht erfüllt. Alte, vor diesem Schritt gespeicherte Presets (flache `PresetCondition[]`) werden von `parseRules` automatisch migriert (`migrateLegacyConditions`), `PresetCondition`/`evaluateCondition`/`parseConditions`/`applyConditionsToSubset` selbst bleiben unverändert bestehen — sie werden weiterhin von den Auto-Verschlagwortungsregeln (`MetadataDialog.tsx`s `createTagRule`) benutzt, einem eigenen, außerhalb dieses Schritts liegenden Feature.
