...
- [x] 3. Masken-/Pinsel-Integration + Batch-Anwendung auf Mehrfachauswahl — eigene `LutFilterStroke`-Liste statt bestehender `Mask`-Infrastruktur (Farbraum-Konflikt: Masken laufen linear, LUTs sind für sRGB gedacht — siehe ADR-0043-Nachtrag), `LutFilterOverlay.tsx` (identisches Pinsel-Muster wie `LiquifyOverlay`). Batch: `lut_filter` in `PRESET_SECTION_KEYS` aufgenommen (treibt Preset-Speichern + Synchronisieren automatisch), `strokes` beim Preset-Kopieren bewusst herausgeschnitten (bildpositions-spezifisch, nicht übertragbar)
- [x] 4. Video als Katalog-Asset (Import, DB-Schema, Thumbnails/Keyframes) — `photos`-Tabelle um `media_kind`/`duration_ms`/`video_codec`/`has_audio`/`frame_rate` erweitert (Migration `0012_video.sql`) statt eigener Tabelle, `import::video` (Endungs-Erkennung + `ffprobe`-Metadaten, kein neues Crate), Thumbnail-Erzeugung per `ffmpeg`-Frame-Extraktion. 23 `NewPhoto`/`Photo`-Konstruktionsstellen entsprechend ergänzt. Noch ohne sichtbaren UI-Effekt (folgt in Schritt 5)
- [x] 5. Video-Wiedergabe (Wiedergabe, Zeitleisten-Grundgerüst) — abweichend von Schritt 0 kein eigener `centerView`-Wert (echte Prüfung zeigte: es gibt gar keine automatische Grid→Einzelansicht-Navigation in dieser App, ADR-0037); stattdessen macht der bestehende Viewer-Fallback-Zweig in `App.tsx` inhaltsbewusst (`VideoPlayer` statt `Viewer` je nach `media_kind`). Neue `video/<id>`-Route mit echter HTTP-Range-Unterstützung (`206 Partial Content`) im `apx://`-Handler, bewusst am `ImageCache` vorbei. `VideoPlayer.tsx` mit eigener anklickbarer Zeitleiste (Grundlage für Schritt 6). Entwickeln-Panel-Ladeversuch für Video an drei Stellen abgesichert
- [x] 6. Schneiden/Trimmen (In/Out-Punkte, Länge anpassen) — nicht-destruktiv: neuer `trim_video`-Command legt das Schnittergebnis als eigene neue Katalogzeile an (`<stem>_trim[_N].<ext>` im selben Ordner), Original bleibt unangetastet. `ffmpeg -c copy` (verlustfrei) mit automatischem Rückfall auf `-c:v libx264 -c:a aac` (Neucodierung), falls Stream-Copy am gewählten Schnittpunkt scheitert. Metadaten-/Thumbnail-Erzeugung wiederverwendet aus Schritt 4 (`import::video`/`import::thumbnails`, dafür `pub(crate)` gemacht). `VideoPlayer.tsx`s Zeitleiste bekommt Anfang-/Ende-Marker + "aktuelle Position markieren"-Knöpfe statt echter Zieh-Interaktion
- [x] 7. Automatisches Zuschneiden (Szenenwechsel-Erkennung) — nativer ffmpeg-`scdet`-Filter (kein externes Modell, seit ffmpeg 4.3) statt eigener Bild-Differenz-Heuristik; neuer `detect_video_scene_changes`-Command parst `lavfi.scd.time`-Zeitstempel aus `stderr`. Bewusst begrenzt (siehe ADR-0043-Nachtrag): reine Szenengrenzen-Erkennung + "diesen Abschnitt übernehmen"-Vorbelegung von Schritt 6s Trimm-Punkten, kein KI-Highlight-Ranking. `VideoPlayer.tsx`s Zeitleiste zeigt erkannte Wechsel als Marker
- [x] 8. Geräuschreduktion + Musik/Sounds hinzufügen — Geräuschreduktion bewusst nur über `afftdn` (native FFT-Entrauschung, kein Modell), nicht das ebenfalls recherchierte `arnndn` (hätte einen eigenen Opt-in-Modell-Download gebraucht, siehe ADR-0043-Nachtrag) — neuer `denoise_video_audio`-Command, drei feste Stufen. Musik hinzufügen (`add_video_audio_track`) nutzt dieselbe Audio-Mix-Technik wie die Diashow-Musikuntermalung (ADR-0034), zwei Modi (mischen/ersetzen), Dateiauswahl über die bereits bestehende generische `pick_file_path`-Dialog-Infrastruktur. Beide nicht-destruktiv wie Schritt 6; `trim_video`s Zielpfad-/Katalog-Logik dafür in zwei geteilte Funktionen gezogen
- [x] 9. Filter/LUT auf Video anwenden — dieselbe trilineare `.cube`-Interpolation wie bei Fotos (`stages::lut_filter::apply`), framegenau über zwei gekoppelte `ffmpeg`-Subprozesse (roh dekodieren → Rust-Thread wendet LUT pro Frame an → re-kodieren, Original-Tonspur per zweitem Input unverändert übernommen). Bewusst global (keine Pinselstriche wie bei Fotos). Frontend nutzt dieselbe Filter-Bibliothek + `.cube`-Import wie das Foto-Entwickeln-Panel. Performance auf langen/hochauflösenden Videos ehrlich unverifiziert (kein Testclip in der Sandbox, siehe ADR-0043-Nachtrag)
- [x] 10. Ähnliche Videos finden — arbeitet exakt wie der bestehende Perceptual-Hash-Duplikat-Assistent für Fotos (`list_perceptual_duplicate_groups`, Phase 9 Schritt 1), beschränkt auf Videos, gehasht wird dasselbe bei Import per ffmpeg extrahierte Vorschau-Frame (kein zweiter Keyframe-Extraktionsweg). Neuer schlanker `SimilarVideoDto`-Wrapper (statt `folder_id` auf das breit genutzte zentrale `PhotoDto` selbst zu heben) trägt den Zielordner für eine neue `jumpToVideo`-Store-Aktion (wechselt bei Bedarf zuerst den Ordner, dann das Foto — auch über Ordnergrenzen hinweg)
- [x] 11. Dokumentation, volle Verifikation, Abnahme — `FEATURES.md` neue Rubrik "Filter-/LUT-Bibliothek + Video-Grundfunktionen (Phase 16)" (alle zehn Schritte), `THIRD_PARTY.md`-Phase-16-Sektion (keine neue Abhängigkeit). Grün: `cargo fmt --all -- --check`, `cargo clippy --workspace --all-targets` (Standard und `--features people`) mit `-D warnings -D clippy::unwrap_used`, `cargo test --workspace`, `tsc -b`, volle `vitest run`-Suite (223 Tests). Zusätzlich real gegen ein per `ffmpeg` erzeugtes Testvideo geprüft (nicht nur kompiliert): Trim (Stream-Copy an Keyframe-Grenze + exakter Re-Encode-Fallback), `scdet`-Szenenerkennung, `afftdn`-Entrauschung, Musik mischen/ersetzen, und der komplette Zwei-Prozess-Rohframe-Pipe-Aufbau für die Video-LUT-Anwendung — alle fünf liefen mit den exakten Befehlszeilen aus dem Code fehlerfrei durch. Playwright: die drei am direktesten von den Datenmodell-Änderungen betroffenen Spezifikationen (`develop-flow`, `library-flow`, `presets-flow`, 39 Tests) liefen grün; ein Lauf der kompletten Suite wurde begonnen, aber auf Nutzeranweisung ("ohne große Tests, es läuft ja alles") nicht abgewartet/abgebrochen — ehrlich vermerkt statt stillschweigend als vollständig ausgegeben. Kein neues Playwright-Video-/LUT-Spezifikat (Schritte 1–10 haben keine `tauri-mock.ts`-Mocks für die neuen Commands bekommen)

## Nachtrag (außerhalb der Phasenfolge): Foto-Globus + Dichte-Heatmap

Direkt im Anschluss an Phase 16 auf expliziten Nutzerwunsch umgesetzt,
ohne eigene Phasennummer (kein mehrschrittiges Vorhaben, eine
zusammenhängende UI-Erweiterung der bestehenden Kartenansicht) — siehe
`DECISIONS.md` ADR-0044 für die vollständige Architektur-Begründung.

- [x] Foto-Globus (`GlobeView.tsx`) — selbst implementierte, drehbare orthographische 3D-Kugelprojektion (`lib/geoProjection.ts`, unit-getestet), Landmasse aus einmalig konvertierten, gebündelten Natural-Earth-110m-Daten (`frontend/src/assets/world-land-110m.json`, keine Laufzeit-Abhängigkeit), Foto-Dichte-Heatmap auf der Oberfläche, automatische Rotation (respektiert `prefers-reduced-motion`/`uiSettings.reduced_motion`), Zieh-Interaktion
- [x] Dichte-Heatmap auf der flachen Karte — selbst implementierter Leaflet-Canvas-Layer (`lib/leafletHeatmap.ts`, statt einer neuen `leaflet.heat`-Abhängigkeit), geteilte Rasterungs-/Farbskala-Logik mit dem Globus (`lib/photoHeatmap.ts`, unit-getestet), Farben aus den Theme-Tokens (Akzent-/Warnfarbe) abgeleitet — folgt Theme-/Akzentfarben-Wechsel automatisch
- [x] Zoom-abhängige Ebenen — Heatmap bis `PIN_ZOOM_THRESHOLD = 10`, einzelne Foto-Pins (bestehendes Marker-/Popup-/Reiserouten-Verhalten) erst bei sehr großem Zoom
