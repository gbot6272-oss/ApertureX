 ...
 * Diashow-Dialog (Einstellungen, Intro/Outro, Musikauswahl), den Aufruf
 * der Live-Wiedergabe (`SlideshowPlayer.tsx`) sowie den optionalen
 * Video-Export über ein simuliertes System-`ffmpeg`. Die eigentliche
 * Ken-Burns-/Zeitachsen-Mathematik ist bereits vollständig in
 * `lib/slideshow.test.ts` (Vitest) abgedeckt, die Frame-/Übergangs-
 * Komposition des Video-Exports in `apx-export`s Rust-Unit-Tests
 * (`video.rs`) — hier nur die Frontend-Verdrahtung.
 */
test.describe("Diashow (Phase 8 Schritt 4)", () => {
  ...
  test("Diashow-Knopf ist ohne Auswahl deaktiviert", async ({ page }) => {
    await installTauriMock(page, {
      folders: [{ id: FOLDER_ID, path: FOLDER_PATH, photo_count: 1 }],
      photosByFolder: { [FOLDER_ID]: [PHOTO] },
    });
  ...
  test("Übergang 'Harter Schnitt' blendet die Übergangsdauer aus", async ({ page }) => {
    await setUpWithSelectedPhoto(page);
    await page.getByRole("button", { name: "Diashow…" }).click();
    await expect(page.getByLabel("Dauer der Überblendung (s)")).toBeVisible(); // Vorgabe: Überblendung
    await page.getByLabel("Übergang").selectOption("cut");
    await expect(page.getByLabel("Dauer der Überblendung (s)")).toHaveCount(0);
  });
  ...
  test("Video-Export sendet die gewählten Einstellungen und zeigt das Ergebnis", async ({ page }) => {
    await setUpWithSelectedPhoto(page, { pickSaveFilePathResult: "/home/user/Videos/Diashow.mp4" });
    await page.getByRole("button", { name: "Diashow…" }).click();
    ...
    const args = call?.args as {
      photoIds: string[];
      destPath: string;
      options: { slideSeconds: number; width: number; height: number; fps: number; transition: string };
    };
    ...
    expect(args.options.transition).toBe("cross_fade"); // Vorgabewert
  });

  test("Ken-Burns-Effekt lässt sich abschalten und wird mitgeschickt", async ({ page }) => {
    await setUpWithSelectedPhoto(page, { pickSaveFilePathResult: "/home/user/Videos/Diashow.mp4" });
    await page.getByRole("button", { name: "Diashow…" }).click();
    ...
    const log = await getMockInvokeLog(page);
    const call = log.find((e) => e.cmd === "export_slideshow_video");
    const args = call?.args as { options: { kenBurns: boolean } };
  ...
  test("Musikauswahl zeigt den Pfad an und wird an den Export weitergereicht", async ({ page }) => {
    await setUpWithSelectedPhoto(page, {
      pickSaveFilePathResult: "/home/user/Videos/Diashow.mp4",
    ...
    await expect(page.getByPlaceholder("Keine ausgewählt")).toHaveValue("/home/user/Musik/song.mp3");

    await page.getByRole("button", { name: "Als Video exportieren (MP4)" }).click();
    const log = await getMockInvokeLog(page);
    const call = log.find((e) => e.cmd === "export_slideshow_video");
    const args = call?.args as { options: { musicPath?: string } };
