...
- [x] Stapel-Metadatenbearbeitung, IPTC-artige Felder (Titel/Bildunterschrift/Copyright/Urheber), Sidecar-Export (.xmp) — Phase 9 — Status: Fertig (abweichend, siehe PLAN.md Schritt 2) — `photos.title`/`caption`/`copyright`/`creator` editierbar über `MetadataDialog.tsx` (Mehrfachauswahl aktualisiert alle markierten Fotos); `apx_export::xmp::write_sidecar` schreibt eine `.xmp`-Sidecar-Datei mit diesen Feldern + Schlagworten; Metadaten-Presets über die generische `templates`-Tabelle und ein vollständiger EXIF/IPTC/XMP-Editor mit frei definierbaren Feldern zurückgestellt (additive Ergänzung ohne Schema-Änderung, keine Blockade für spätere Schritte)
...
- [x] Personenansicht (Gesichtserkennung) — Phase 9, in Phase 11 Schritt 5 nachgerüstet — Status: Fertig (abweichend, siehe DECISIONS.md ADR-0038) — `apx_ai::faces::detect_face_regions`: Hautton-Erkennung im YCbCr-Raum (wiederverwendet `segmentation::person_alpha`) plus Connected-Component-Konturanalyse liefert grobe Bounding-Boxes; `list_people_groups`-Command gruppiert Fotos nach Blob-Anzahl/-Fläche-Signatur, neuer `centerView`-Modus `"people"` (`PeopleView.tsx`) zeigt die Gruppen — **ehrlich begrenzt**: keine echte Gesichts-/Personen-Identifizierung, nur eine grobe Vorsortierung
...
- [ ] Zielgerichtetes Anpassungswerkzeug (TAT) für Kurven und HSL — Phase 9 — Status: Zurückgestellt (siehe PLAN.md Schritt 5) — welcher Regler an welche Zugrichtung gebunden wird, ist ohne Lightroom 1:1 zu kopieren mehrdeutig; eine eigene, durchdachte Interaktions-UX-Iteration statt einer überstürzten Umsetzung
...
- [ ] Export Adobe `.lrtemplate` — Phase 11 — Status: Zurückgestellt (siehe PLAN.md Phase 11 Schritt 8) — nicht offiziell dokumentiertes Format, nur Export (kein Import) geplant, best-effort-Mapping wie beim `.xmp`-Crs-Format
...
- [ ] Stapelverarbeitungs-Konsole (Vorschau, Trockenlauf, Rückgängig) — Phase 9 — Status: Zurückgestellt (siehe DECISIONS.md ADR-0036) — war in ADR-0035s Scope-Aufzählung, bekam aber nie einen eigenen Bauschritt zugeteilt (Lücke im Schritt-0-Zuschnitt, bei der Schritt-12-Abnahme aufgefallen); ein ehrliches Rückgängig-Machen der gesamten Aktion bräuchte einen neuen feldübergreifenden Batch-Operationen-Log — derselbe Umfang wie ein eigener Bauschritt, bewusst nicht überstürzt in Schritt 12 nachgeflanscht
...
- [x] Tethered Shooting (gphoto2/PTP, Live-View, Auto-Preset) — Phase 9 — Status: Fertig (abweichend, siehe PLAN.md Schritt 11) — **kein Live-View** (nicht Teil des in `PLAN.md` beschriebenen Kernablaufs Kamera erkennen → auslösen → herunterladen → Auto-Preset); `Gphoto2Backend`s echte `libgphoto2`-FFI-Aufrufe sind geschrieben, aber in dieser Sandbox/im Standard-CI nie gegen echte Hardware oder auch nur eine installierte `libgphoto2`-Bibliothek ausführbar (ADR-0035 Punkt 5) — ohne das Cargo-Feature `tethering` (Standard) läuft ausschließlich eine klar als Simulation markierte `FakeBackend`
...
- [x] Paletten ein-/ausklappbar, breitenziehbar, Arbeitsbereich-Preset speicherbar — Phase 3 — Status: Fertig (abweichend, in Phase 10 Schritt 3 umgesetzt) — `PaletteFrame.tsx` (Ziehgriff 75–480px, Einklapp-Knopf, `localStorage`) auf Sidebar/Presets/Metadaten-Panel angewendet; „Arbeitsbereich zurücksetzen" im neuen `SettingsDialog.tsx`. **Teil-Einschränkung:** `DevelopPanel`/`MasksPanel` bleiben ohne Ziehen/Einklappen (Fragment-umschlossen mit Geschwister-Dialogen, am dichtesten e2e-getestete Dateien im Frontend — ihr bestehendes store-gesteuertes Ein-/Ausblenden über den Header bleibt ihre Form von „ausklappbar")
...
- [x] Vollständig belegbare Tastenkürzel + Cheatsheet-Overlay (`?`) — Phase 10 — Status: Fertig (abweichend, siehe PLAN.md Schritt 5) — `lib/keybindings.ts`: zentrale, umbelegbare Zuordnungstabelle für die globalen `App.tsx`-Kürzel (Bildwechsel/Palette/Undo-Redo/Vollbild/Flaggen/Cheatsheet), Neubelegung in `localStorage`; Bewertungs-Zifferntasten (0–5, parametrisiert) bleiben bewusst fest. **Teil-Einschränkung:** lokale Komponenten-Kürzel (Viewer-Zoom, DevelopPanel-Undo/Redo, Slider-Feinjustierung per Pfeiltasten) sind rein informativ im Cheatsheet gelistet, nicht umbelegbar — die am dichtesten getesteten Pfade im Frontend, siehe `lib/keybindings.ts`s Moduldoku
...
- [x] Lokalisierung (Deutsch und Englisch) — Phase 10 — Status: Fertig (abweichend, siehe PLAN.md Schritt 8) — `lib/i18n.ts` + `lib/locales/de.ts`/`en.ts`, Header/Sidebar/Presets-/Metadaten-Panel/Einstellungen/Cheatsheet übersetzt. **Teil-Einschränkung:** die ca. 20 Dialog-Komponenten (Export/Druck/Diashow/Buch/Web/Vorlagen/Organisieren/Stacking/Skript/Kollaboration/Tethering/Metadaten-Editor/Statistik), die den Großteil der ~10.700 Frontend-Zeilen tragen, bleiben unübersetzt — offene Ausbaustufe
...
- [x] Installer und Signierung für alle drei Plattformen — Phase 10 — Status: Fertig (abweichend, siehe PLAN.md Schritt 11) — `@tauri-apps/cli` + neuer CI-`release`-Job auf dem bestehenden 3-OS-Matrix (nur bei Tag-Push/`workflow_dispatch`), Signierungskonfiguration strukturell vorbereitet und konditional auf GitHub-Secrets (macOS: Zertifikat+Notarisierung direkt aus der Umgebung gelesen; Windows: PFX-Import + Fingerabdruck-Config-Override) — überspringt sich selbst ohne Fehlschlag, wenn Secrets fehlen. **Ehrlich begrenzt:** kein echtes Zertifikat/Apple-Developer-Konto in dieser Sandbox beschaffbar oder verifizierbar, erzeugte Installer bleiben bis zur Hinterlegung eigener Secrets unsigniert
