...
- [x] Foto-Globus + Dichte-Heatmap — Nachtrag (siehe `DECISIONS.md` ADR-0044) — Status: Fertig — drei Ebenen: ein selbst gerenderter, drehbarer 3D-Globus (orthographische Projektion, Canvas 2D) als Standard-Einstieg mit einer Foto-Dichte-Heatmap auf der Oberfläche; dieselbe Heatmap auf der flachen Karte (dunkle CARTO-Kacheln) bei mittlerem Zoom; einzelne Pins erst ab sehr großem Zoom. Heatmap-Farben aus den Theme-Tokens (Akzent-/Warnfarbe) abgeleitet, folgen Theme-/Akzentfarben-Wechsel automatisch
...
- [x] Rechte Werkzeug-Palette, Modul-Umschalter oben — Phase 3 — Status: Fertig (abweichend, in Phase 10 Schritt 2 umgesetzt, in Phase 18 Schritt 3 auf eine schlanke Kopfleiste umgebaut, siehe DECISIONS.md ADR-0037/ADR-0046) — `Header.tsx` jetzt **eine** Zeile statt zwei: Logo, Import, Such-/Befehlsknopf (öffnet die Befehlspalette), die sechs Ansicht-Ziele als zusammenhängende Segment-Gruppe (Raster/Übersicht/Karte/Personen/Info/Entwickeln), ein Overflow-Menü (`components/ui/Menu.tsx`) für die vier vormaligen Zeile-2-Gruppen (Ausgabe/Vorlagen/Fortgeschritten/Analyse, jetzt aus `lib/commandRegistry.ts` gespeist statt zwei parallel gepflegter Listen) sowie Einstellungen; kein Lightroom-artiger vollständiger Bildschirmwechsel pro Modul — jeder Eintrag öffnet unverändert denselben Dialog wie zuvor, nur anders erreichbar. `DevelopPanel`/`MasksPanel` unter gemeinsamer visueller Außenhülle als rechte Werkzeug-Palette, seit Phase 18 Schritt 4 zusätzlich je in Registerkarten gegliedert (siehe unten)
...
- [x] Paletten ein-/ausklappbar, breitenziehbar, Arbeitsbereich-Preset speicherbar — Phase 3/11 — Status: Fertig (abweichend, in Phase 10 Schritt 3 umgesetzt, Phase 11 Schritt 11 vervollständigt, Phase 18 Schritt 5 mit sanfter Breitenanimation ergänzt) — `PaletteFrame.tsx` (Ziehgriff 75–480px, Einklapp-Knopf, `localStorage`, seit Phase 18 zusätzlich eine `usePrefersReducedMotion()`-gesicherte Breiten-Übergangsanimation beim Ein-/Ausklappen) jetzt auf allen sechs Paletten (Sidebar/Presets/Metadaten-Panel/**Entwickeln/Masken**); „Arbeitsbereich zurücksetzen" im `SettingsDialog.tsx`. Direkt nach der `DevelopPanel`/`MasksPanel`-Umstellung volle `develop-flow.spec.ts`/`masks-flow.spec.ts`-Regression gefahren (40 Tests grün) — dieselbe Vorsicht, die Phase 10 Schritt 3 zurückgestellt hatte, jetzt mit Testnetz. `DevelopPanel`/`MasksPanel` selbst seit Phase 18 Schritt 4 zusätzlich in fünf (bzw. drei) Registerkarten gegliedert (Licht/Farbe/Details/Kreativ/Verlauf & Werkzeuge) statt einer durchgehenden Scroll-Spalte, siehe DECISIONS.md ADR-0046.
...
- [x] Befehlspalette — vollständig (jede Funktion/jedes Preset) — Phase 5 — Status: Fertig (abweichend, in Phase 10 Schritt 4 umgesetzt, in Phase 18 Schritt 3 lokalisiert und registergespeist) — alle Header-Funktionen (inkl. der neun weiterhin `Header.tsx`-lokalen Dialoge über eine `pendingCommand`-Store-Brücke), alle Presets (wendet an), alle Fotos der aktuellen Ansicht (wählt aus), Ordner (bestehend). Liest seit Phase 18 aus `lib/commandRegistry.ts` (derselben Quelle wie das Kopfleisten-Overflow-Menü) statt eigener hartcodierter Einträge, Labels über `useT()`; neue Kategorie „KI-Funktionen" listet alle neun Opt-in-KI-Features mit Modellstatus-Badge (Bereit/Download nötig/Nicht verfügbar) und springt direkt zum passenden Panel/Modus — behebt das zuvor gefundene Auffindbarkeits-Problem des KI-Ausfüllen-Downloads
...
- [x] Einheitliches, hochwertig wirkendes Design-System (Dialoge, Bewegung) — Phase 18 — Status: Fertig (Nutzerurteil nach eigenem Test: „wirkt wie eine billige App", siehe DECISIONS.md ADR-0046) — `components/ui/Dialog.tsx`/`Sheet.tsx` ersetzen 25 vormals unabhängig handgerollte Dialog-Hüllen (uneinheitlicher Backdrop, nur 3/25 mit Fokus-Falle) durch eine gemeinsame, dezente Overlay-/Ein-Ausblend-Primitive (`--color-bg-overlay`, `Sheet` kantenseitig für Export/Zeitachse/Stapelverarbeitung statt zentriert — fühlt sich als Teil des Arbeitsflusses an, nicht als Unterbrechung); erweiterte Radius-/Schatten-/Bewegungs-Tokens in `index.css`s `@theme`; globale Hover-/Fokus-Übergänge auf allen interaktiven Elementen, sanfter Überblend-Wechsel beim Ansicht-Wechsel, Anwenden-Rückmeldung bei Preset-Übernahme; `usePrefersReducedMotion()` (`lib/motion.ts`) schaltet jede JS-getimte Bewegung zusätzlich zur bestehenden CSS-`prefers-reduced-motion`-Bremse ab
