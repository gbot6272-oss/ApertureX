...
import type { LiquifyPoint, LiquifyStroke } from "../lib/edl";

interface LiquifyOverlayProps {
  /** Position/Größe des angezeigten Bildes in Bildschirm-Pixeln, wie
   * `RepairOverlay`s gleichnamige Props (siehe dort). */
  imageLeft: number;
  imageTop: number;
  imageWidth: number;
  imageHeight: number;
  ...
  radius: number;
  onPaint: (path: LiquifyPoint[]) => void;
  onRemoveStroke: (index: number) => void;
}

/** Muss `stages::liquify`s implizites Limit widerspiegeln — dieselbe
 * Ausdünnung wie `RepairOverlay`s `MAX_TARGET_PATH_POINTS`, hier separat
 * gehalten (siehe dessen Begründung). */
const MAX_CENTER_PATH_POINTS = 32;

function thinPath(path: LiquifyPoint[]): LiquifyPoint[] {
  if (path.length <= MAX_CENTER_PATH_POINTS) return path;
  const step = (path.length - 1) / (MAX_CENTER_PATH_POINTS - 1);
  const thinned: LiquifyPoint[] = [];
...
function pointFromEvent(event: { clientX: number; clientY: number }, rect: DOMRect): LiquifyPoint {
  return {
...
/**
 * Verflüssigen-Overlay (Phase 15 Schritt 3, siehe `DECISIONS.md`
 * ADR-0042 — Photoshop-exklusiv, Lightroom hat kein Verformungswerkzeug).
 * Anders als der Reparatur-Pinsel gibt es keinen Quellpunkt-Schritt: ein
 * Ziehvorgang malt direkt den Verformungspfad, beim Loslassen sofort als
 * fertiger Strich committet (dieselbe „kein separates Anwenden nötig"-
 * Vereinfachung wie bei jeder rein deterministischen CPU-Stufe dieses
 * Projekts).
 */
export function LiquifyOverlay({ imageLeft, imageTop, imageWidth, imageHeight, strokes, radius, onPaint, onRemoveStroke }: LiquifyOverlayProps) {
  const [drawingPath, setDrawingPath] = useState<LiquifyPoint[] | null>(null);
  const [hoverPoint, setHoverPoint] = useState<LiquifyPoint | null>(null);
  const pathRef = useRef<LiquifyPoint[]>([]);
      ...
      {strokes.map((stroke, index) => {
        ...
        return (
          <button
            key={index}
            type="button"
            aria-label={`Verflüssigen-Strich ${index + 1} entfernen`}
            onClick={(event) => {
